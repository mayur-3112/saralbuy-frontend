...
const Requirement = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 min-h-screen relative">
      ...
      <div className="mt-10 mb-8 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        <div 
          className={`border-2 rounded-lg p-6 cursor-pointer shadow-sm hover:shadow-md transition-all ${
            postMode === 'single' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300'
          }`}
          onClick={() => {
            setPostMode('single');
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className={`p-2 rounded-full ${postMode === 'single' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
            ...
            <h2 className={`text-xl font-bold ${postMode === 'single' ? 'text-blue-600' : 'text-gray-800'}`}>Post a Single Requirement</h2>
          </div>
        ...
        <div 
          className={`border-2 rounded-lg p-6 cursor-pointer shadow-sm hover:shadow-md transition-all ${
            postMode === 'upload' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300'
          }`}
          onClick={() => {
            setPostMode('upload');
            window.scrollTo({ top: 300, behavior: 'smooth' });
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className={`p-2 rounded-full ${postMode === 'upload' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
            ...
            <h2 className={`text-xl font-bold ${postMode === 'upload' ? 'text-blue-600' : 'text-gray-800'}`}>Upload PDF Requirement</h2>
          </div>
          ...
          {data &&
            data?.map(item => (
              <AccordionItem value={item?._id} key={item._id}>
                <AccordionTrigger className="capitalize px-4 bg-white border border-slate-200 rounded-lg hover:border-blue-300 transition-colors">
                  <div className="flex items-center gap-x-4">
                    <div className="w-10 h-10 rounded-md bg-blue-50 text-blue-500 flex items-center justify-center font-bold text-lg">
                ...
                <AccordionContent className="flex flex-col gap-1 pl-4">
                  {item?.subCategories?.map(sub => (
                    <p
                      ...
                      key={sub?._id}
                      className="capitalize text-slate-600 hover:text-blue-500 py-2 border-b border-slate-100 text-sm cursor-pointer last:border-none"
                    >
                      {sub?.name}
                    </p>
                  ))}
      ...
      {/* looking for div */}
      <div className="bg-blue-50 p-7 rounded-[5px] my-6">
        ...
        <div className="flex justify-between items-center m-1">
          ...
          <button onClick={handleOtherClick} className="cursor-pointer hover:bg-blue-100 p-2 rounded-full transition-colors">
